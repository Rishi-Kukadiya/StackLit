...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-[#2E236C]/50 backdrop-blur-xs shadow-xl z-40 flex flex-col pt-6 px-3 transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:flex lg:w-64`}
        style={{ borderRight: "1.5px solid #433D8B", marginTop: open ? '0px' : '64px' }}
      >
        ...
        <nav className="overflow-y-auto scrollbar-thin scrollbar-thumb-[#433D8B] scrollbar-track-transparent">
          {sidebarOptions.map((opt) => (
            ...
            >
              <span className="w-5 h-5 flex items-center justify-center">{opt.icon}</span>
              {opt.label}
            </Link>
          ))}
