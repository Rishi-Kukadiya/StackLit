import { Tag, Eye, ThumbsUp, Clock, User, ArrowRight } from "lucide-react";
  ...
  <div className="fixed top-0 w-full z-50">
      ...
      <div className="pt-16 min-h-screen overflow-y-auto relative z-10 sm:ml-64 transition-all duration-300">
        <div className="max-w-4xl mx-auto p-3 sm:p-4 md:p-6 lg:p-8">
          <div className="bg-[#2E236C]/20 backdrop-blur-sm rounded-lg 
                shadow-[0_0_20px_rgba(200,172,214,0.2)]
                border border-[#C8ACD6]/20
                p-4 sm:p-6
                relative
                before:absolute before:inset-0 
                before:bg-gradient-to-r before:from-[#433D8B]/10 before:to-transparent 
                before:rounded-lg before:pointer-events-none">
          {/* Row 1: User Info Section */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6 border-b border-[#433D8B] pb-4">
          ...
          <div className="flex-grow w-full sm:w-auto">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {/* <span className="text-[#C8ACD6] text-sm">
          ...
          <div className="text-[#C8ACD6] space-y-4 mb-6 text-sm sm:text-base">
            {question.content.split('```').map((block, index) => {
              if (index % 2 === 1) { // Code block
                return (
                  <pre key={index} className="bg-[#17153B] p-3 sm:p-4 rounded-lg overflow-x-auto text-xs sm:text-sm">
        ...
        <div className="space-y-4 pt-4 border-t border-[#433D8B]">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              {question.tags.map((tag) => (
                <span 
                  key={tag}
                  className="flex items-center gap-1 px-3 py-1 bg-[#433D8B]/50 text-[#C8ACD6] rounded-full text-sm
                           hover:bg-[#17153B]/70 hover:text-white transition-all duration-300 cursor-pointer"
                >
